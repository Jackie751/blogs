      ...
      {a.cover ? (
        <div style={{position:'relative', width:'100%', paddingTop: mobile ? '125%' : '145%', overflow:'hidden'}}>
          ...
          <div style={{position:'absolute',top:16,left:16,display:'flex',gap:8,alignItems:'center'}}>
            {isCurrent && (
              <span style={{fontFamily:'monospace',fontSize:8,padding:'2px 8px',letterSpacing:'.15em',background:'rgba(255,255,255,0.12)',color:'rgba(255,255,255,0.7)',backdropFilter:'blur(8px)',borderRadius:2}}>
                LATEST
              </span>
            )}
            <span style={{fontFamily:'monospace',fontSize:8,padding:'2px 8px',letterSpacing:'.15em',color:ac,background:`${ac}18`,backdropFilter:'blur(8px)',borderRadius:2}}>
          ...
          {/* 右上阅读时间 */}
          {a.read_time && (
            <div style={{position:'absolute',top:16,right:16,fontFamily:'monospace',fontSize:8,color:'rgba(255,255,255,0.45)',letterSpacing:'.1em'}}>
              {a.read_time} MIN
            </div>
          )}
          ...
          <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'0 20px 0'}}>
            <div style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.3)',letterSpacing:'.15em',marginBottom:8}}>
              {(a.date||'').replace(/-/g,' · ')}
            </div>
            <h2 style={{
            ...
            {a.tags?.length > 0 && (
              <div style={{display:'flex',gap:6,marginBottom:10,flexWrap:'wrap'}}>
                {a.tags.slice(0,3).map(t => (
                  <span key={t} style={{fontFamily:'monospace',fontSize:8,color:'rgba(255,255,255,0.4)',letterSpacing:'.1em'}}>
                    #{t}
                  </span>
                ))}
              </div>
        ...
        <div style={{padding:'28px 20px 0',borderTop:`1px solid ${ac}44`}}>
          <div style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.25)',letterSpacing:'.15em',marginBottom:12}}>
          ...
          <div style={{margin:'14px -20px 0',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
            <button onClick={() => navigate(`/article/${a.folder}/${a.id}`)}
              style={{width:'100%',padding:'12px 0',background:'transparent',border:'none',color:'rgba(255,255,255,0.4)',fontFamily:'monospace',fontSize:10,letterSpacing:'.2em',cursor:'pointer'}}>
              READ →
            </button>
          </div>
        </div>
      ...
      <div style={{background:'#060608',minHeight:'100vh',color:'#e8ddd0',fontFamily:"'Noto Sans SC',sans-serif"}}>
        ...
        <div style={{padding:'20px 20px 16px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:16}}>
            <div>
              <div style={{fontFamily:"'Noto Serif SC',serif",fontSize:26,fontWeight:700,color:'#f0e8d8',letterSpacing:'-.01em',lineHeight:1}}>柒安的小窝</div>
              <div style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.25)',letterSpacing:'.2em',marginTop:4}}>PERSONAL WRITING</div>
            </div>
            <div style={{display:'flex',gap:12,alignItems:'center'}}>
              ...
              <a href="https://index.jackie3137.xyz" target="_blank" rel="noopener"
                style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.3)',textDecoration:'none',letterSpacing:'.1em'}}>HOME</a>
            </div>
          </div>
          {searchOpen && (
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索..."
              style={{width:'100%',padding:'8px 0',background:'transparent',border:'none',borderBottom:'1px solid rgba(255,255,255,0.15)',color:'#e8ddd0',fontFamily:"'Noto Serif SC',serif",fontSize:14,outline:'none',boxSizing:'border-box'}} />
          )}
        </div>

        {/* 分类 */}
        <div style={{padding:'14px 20px',display:'flex',gap:20,overflowX:'auto',scrollbarWidth:'none',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
          {CATS.map(c => (
            <button key={c.key} onClick={() => setTab(c.key)}
              style={{
                background:'none', border:'none', cursor:'pointer',
                fontFamily:'monospace', fontSize:9, letterSpacing:'.2em',
                color: tab===c.key ? '#fff' : 'rgba(255,255,255,0.25)',
        ...
        {/* 文章数 */}
        <div style={{padding:'12px 20px',fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.2)',letterSpacing:'.15em',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
          {visible.length} ARTICLES
        </div>

        {/* 列表 */}
        <div style={{padding:'20px',display:'flex',flexDirection:'column',gap:20}}>
          {loading && <p style={{color:'rgba(255,255,255,0.2)',fontFamily:'monospace',fontSize:10,textAlign:'center',padding:'60px 0',letterSpacing:'.2em'}}>LOADING</p>}
          {!loading && visible.length === 0 && <p style={{color:'rgba(255,255,255,0.2)',fontFamily:'monospace',fontSize:10,textAlign:'center',padding:'60px 0',letterSpacing:'.2em'}}>NO ARTICLES</p>}
          {visible.map(a => {
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100vh',background:'#060608',color:'#e8ddd0',fontFamily:"'Noto Sans SC',sans-serif"}}>
      ...
      <div style={{display:'flex',alignItems:'center',padding:'18px 48px',borderBottom:'1px solid rgba(255,255,255,0.05)',flexShrink:0,zIndex:20}}>
        <div style={{display:'flex',alignItems:'baseline',gap:16}}>
          <div style={{fontFamily:"'Noto Serif SC',serif",fontSize:22,fontWeight:700,color:'#f0e8d8',letterSpacing:'-.01em'}}>柒安</div>
          <div style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.2)',letterSpacing:'.25em'}}>PERSONAL WRITING</div>
        </div>

        {/* 分类 - 放顶栏中间 */}
        <div style={{margin:'0 auto',display:'flex',gap:32,alignItems:'center'}}>
          {CATS.map(c => (
            <button key={c.key} onClick={()=>{setTab(c.key);setCur(0)}}
              ...
              {c.label}
              <span style={{marginLeft:6,fontSize:7,color:'rgba(255,255,255,0.15)'}}>{count(c.key)}</span>
            </button>
          ))}
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:24}}>
          ...
          <a href="https://index.jackie3137.xyz" target="_blank" rel="noopener"
            style={{fontFamily:'monospace',fontSize:9,color:'rgba(255,255,255,0.25)',textDecoration:'none',letterSpacing:'.15em',transition:'color .2s'}}
            onMouseEnter={e=>e.currentTarget.style.color='#fff'}
      ...
      {/* 文章数小标 */}
      <div style={{padding:'8px 48px',fontFamily:'monospace',fontSize:8,color:'rgba(255,255,255,0.15)',letterSpacing:'.2em',flexShrink:0,borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
        {visible.length} ARTICLES
      </div>

      {/* 轮播舞台 */}
      <div style={{flex:1,position:'relative',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
        {loading && <p style={{color:'rgba(255,255,255,0.2)',fontFamily:'monospace',fontSize:9,letterSpacing:'.3em'}}>LOADING</p>}
        {!loading && visible.length===0 && (
          <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <p style={{color:'rgba(255,255,255,0.15)',fontFamily:'monospace',fontSize:9,letterSpacing:'.3em'}}>NO ARTICLES</p>
          </div>
        )}

        <button onClick={()=>move(-1)} disabled={visible.length<=1}
          style={{position:'absolute',left:20,top:'50%',transform:'translateY(-50%)',zIndex:30,background:'none',border:'none',color:'rgba(255,255,255,0.2)',fontSize:28,cursor:'pointer',padding:'8px',transition:'color .2s',opacity:visible.length<=1?0:1}}
          onMouseEnter={e=>e.currentTarget.style.color='#fff'}
        ...
        <button onClick={()=>move(1)} disabled={visible.length<=1}
          style={{position:'absolute',right:20,top:'50%',transform:'translateY(-50%)',zIndex:30,background:'none',border:'none',color:'rgba(255,255,255,0.2)',fontSize:28,cursor:'pointer',padding:'8px',transition:'color .2s',opacity:visible.length<=1?0:1}}
          onMouseEnter={e=>e.currentTarget.style.color='#fff'}
          onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,0.2)'}>›</button>
      </div>

      {/* 底部 */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:16,padding:'12px 0',borderTop:'1px solid rgba(255,255,255,0.04)',flexShrink:0}}>
        ...
        <span style={{fontFamily:'monospace',fontSize:8,color:'rgba(255,255,255,0.15)',letterSpacing:'.2em'}}>
